...
async function connectToWhatsApp() {
    ...
    sock = makeWASocket({
        ...
        printQRInTerminal: true // لطباعة الـ QR في لوحة تحكم Render عند التشغيل الأول
    });
...
bot.on('document', async (ctx) => {
    ...
    try {
        ...
        for (let num of lines) {
            // تنظيف الرقم من أي رموز مضافة مثل (+) أو مسافات، ليتبقى الأرقام فقط بكود الدولة
            let cleanNum = num.replace(/\D/g, ''); 
            if (!cleanNum) continue;

            try {
                // الفحص الفعلي للرقم بدون إرسال رسائل
        ...
        // حفظ نتائج الفحص مؤقتاً للتحكم بها عبر الأزرار
...
// التحكم بالأزرار التفاعلية (Inline Buttons)
...
// إغلاق آمن للبوت عند إيقاف السيرفر
