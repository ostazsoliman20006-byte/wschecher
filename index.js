...
// دالة تشغيل أو ربط الواتساب لكل مستخدم تلقائياً أو عبر كود
async function initWhatsApp(userId, phoneNumber = null, ctx = null) {
    ...
    const sock = makeWASocket({
        ...
        browser: ["Ubuntu", "Chrome", "20.0.04"] // ضروري لتعريف السيرفر كمتصفح مقبول عند طلب الكود
    });
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (shouldReconnect) {
                ...
                // إذا سجل المستخدم خروج يدوياً يتم تنظيف مخلفات الجلسة
                userSockets.delete(userId);
                if (fs.existsSync(authFolder)) {
                    fs.rmSync(authFolder, { recursive: true, force: true });
                }
                if (ctx) ctx.reply('⚠️ تم تسجيل الخروج من حساب الواتساب الخاص بك بنجاح.');
            }
    ...
    // إذا طلب المستخدم ربطاً جديداً بالرقم
    if (phoneNumber) {
        setTimeout(async () => {
            try {
                let code = await sock.requestPairingCode(phoneNumber);
                // تنسيق الكود ليظهر مثل: XXXX-XXXX
                ...
                console.error(err);
                if (ctx) ctx.reply('❌ فشل توليد كود التحقق. تأكد من إدخال الرقم بشكل صحيح مع رمز الدولة بدون مفتاح (+) أو مسافات، وحاول مجدداً.');
            }
        }, 3000); // مهلة زمنية بسيطة لضمان بدء المقبس بنجاح
    }
}

// القائمة الرئيسية للبوت تماثل واجهة المستخدم المطلوبة
...
bot.start((ctx) => {
    // محاولة تشغيل جلسة المستخدم إذا كانت مسجلة مسبقاً لمنع طلب الكود في كل مرة
...
bot.action('manage_account', async (ctx) => {
    ...
    if (isConnected) {
        const keyboard = Markup.inlineKeyboard([
            [Markup.button.callback('❌ Logout Account', 'logout_wa')],
            [Markup.button.callback('⬅️ Back', 'back_menu')]
        ]);
        ctx.editMessageText('⚙️ *إعدادات الحساب:*\n\nحساب الواتساب الخاص بك متصل حالياً وجاهز للاستخدام.', { parse_mode: 'Markdown', ...keyboard });
    } else {
        const keyboard = Markup.inlineKeyboard([
            [Markup.button.callback('📱 Link via Pair Code', 'request_code')],
            [Markup.button.callback('⬅️ Back', 'back_menu')]
        ]);
        ctx.editMessageText('⚙️ *إعدادات الحساب:*\n\nلم تقم بربط حساب واتساب حتى الآن للبدء في الفحص.', { parse_mode: 'Markdown', ...keyboard });
    }
});

// عند طلب الربط عبر الكود
...
// التعامل مع استقبال رقم الهاتف من المستخدم
bot.on('text', async (ctx, next) => {
    const userId = ctx.from.id;
    const state = userStates.get(userId);

    if (state === 'WAITING_PHONE_NUMBER') {
        let phone = ctx.message.text.replace(/\D/g, ''); // تنظيف النص من أي رموز وترك الأرقام فقط
        if (phone.length < 8) {
            return ctx.reply('❌ الرقم الذي أدخلته غير صحيح. تأكد من كتابة رمز الدولة يليه الرقم مباشرة.');
        }

        userStates.delete(userId);
        await ctx.reply('⏳ جاري الاتصال بخوادم الواتساب وتوليد الكود الخاص بك...');
        
        // تشغيل التهيئة وطلب الكود
...
bot.action('logout_wa', async (ctx) => {
    ...
    if (sock) {
        ...
        ctx.reply('⚠️ تم مسح الجلسة بنجاح.');
    }
    ctx.answerCbQuery();
    sendMainMenu(ctx);
});
...
bot.action('view_help', (ctx) => {
    ctx.reply('ℹ️ للبدء بالفحص: قم بربط حسابك من زر Account، ثم أرسل ملف .txt يحتوي على الأرقام (رقم في كل سطر).');
    ctx.answerCbQuery();
});

// استقبال ومعالجة ملفات الفحص النصية لكل مستخدم بحسابه الخاص
bot.on('document', async (ctx) => {
    const userId = ctx.from.id;
    const sock = userSockets.get(userId);
    
    // التحقق من أن المستخدم قام بربط حسابه أولاً
    if (!sock || sock.ws?.readyState !== 1) {
        return ctx.reply('❌ يجب عليك ربط حساب الواتساب الخاص بك أولاً عبر الضغط على زر Account قبل إرسال ملف الفحص.');
    }
