...
async function initWhatsApp(userId, phoneNumber = null, ctx = null, mode = 'PAIR') {
    ...
    if ((phoneNumber || mode === 'QR') && fs.existsSync(authFolder)) {
        try { fs.rmSync(authFolder, { recursive: true, force: true }); } catch (e) {}
    }

    const { state, saveCreds } = await useMultiFileAuthState(authFolder);

    // محاكاة متصفح مستقر لتفادي الرفض التلقائي من خوادم واتساب
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        // إذا طلب المستخدم كود الـ QR، يتم تحويله لصورة وإرسالها فوراً بالتليجرام
        if (qr && mode === 'QR') {
            ...
            if (ctx) {
                await ctx.replyWithPhoto(qrImageUrl, { 
                    caption: '🔲 *كود الـ QR للربط جاهز الآن:*\n\nافتح واتساب في هاتفك ⬅️ الأجهزة المرتبطة ⬅️ ربط جهاز، ثم وجه كاميرا الهاتف وامسح هذا الكود مباشرة.', 
                    parse_mode: 'Markdown' 
                }).catch(() => {});
            }
        }
        ...
        if (connection === 'close') {
            ...
            if (shouldReconnect) {
                initWhatsApp(userId, null, ctx, mode);
            } else {
                userSockets.delete(userId);
                if (fs.existsSync(authFolder)) {
                    fs.rmSync(authFolder, { recursive: true, force: true });
                }
                if (ctx) ctx.reply('⚠️ تم إلغاء الاتصال أو تسجيل الخروج من الحساب.');
            }
        } else if (connection === 'open') {
            if (ctx) ctx.reply('✅ تم ربط حساب الواتساب الخاص بك بنجاح وهو جاهز تماماً الآن لمباشرة الفحص!');
        }
    });

    if (phoneNumber && mode === 'PAIR') {
        setTimeout(async () => {
            try {
                ...
                if (ctx) {
                    ...
                    await ctx.reply('ℹ️ افتح الواتساب في هاتفك ⬅️ الأجهزة المرتبطة ⬅️ ربط جهاز ⬅️ اختر "الربط برقم الهاتف عوضاً عن ذلك" ثم أدخل الكود أعلاه يدوياً.');
                }
            } catch (err) {
                console.error(err);
                if (ctx) ctx.reply('❌ فشل توليد كود التحقق بالرقم بسبب قيود خوادم الاستضافة. يُنصح بشدة باستخدام خيار الـ QR Code فهو مضمون 100%.');
            }
        }, 3000); 
    }
}
...
async function showAccountManager(ctx, isCommand = false) {
    ...
    const message = isConnected ? '⚙️ *إعدادات الحساب:*\n\nحساب الواتساب الخاص بك متصل حالياً وجاهز للاستخدام.' : '⚙️ *إعدادات الحساب:*\n\nاختر طريقة ربط حساب واتساب الخاص بك للبدء في الفحص:\n\n💡 *نصيحة:* طريقة كود الـ QR هي الأسرع وتتجنب مشاكل رفض خوادم واتساب تماماً.';
    const keyboard = isConnected
        ...
        : Markup.inlineKeyboard([
            [Markup.button.callback('🔲 Link via QR Code (مضمونة وموصى بها)', 'link_qr')],
...
bot.action('link_qr', async (ctx) => {
    await ctx.editMessageText('⏳ جاري تهيئة خوادم واتساب وتوليد كود الـ QR، انتظر لحظة لحين ظهور الصورة...');
    initWhatsApp(ctx.from.id, null, ctx, 'QR');
});
...
bot.on('text', async (ctx, next) => {
    ...
    if (state === 'WAITING_PHONE_NUMBER') {
        ...
        await ctx.reply('⏳ جاري محاولة طلب الكود عبر تهيئة متصفح محسن لتقليل قيود السيرفر...');
        await initWhatsApp(userId, phone, ctx, 'PAIR');
    } else {
        return next();
    }
});
